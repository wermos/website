...
![Figure 2: visualize the noise function as a grid of size 256x256 (for the 2 cases) that is repeated in every direction (in each quadrant of the Cartesian coordinate system). This is only for visualization. In practice, we never repeat the grid. Instead, we take the coordinates of the point modulo the table size (256 in this case).
