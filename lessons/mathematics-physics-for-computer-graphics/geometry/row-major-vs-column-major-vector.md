...
There is another potentially essential aspect to consider if you need to choose between row-major and column-major, but this has nothing to do with the conventions themselves and how practical one is over the other. It has more to do with the computer and the way it works. Remember that we will be dealing with [4x4] matrices. Typically the implementation of a matrix in C++ looks like this:
